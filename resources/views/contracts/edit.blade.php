...
<div class="max-w-3xl mx-auto">
    ...
    <form action="{{ route('contracts.update', $contract) }}" method="POST" class="space-y-6" enctype="multipart/form-data" onsubmit="return validateEditContractForm(event)"
          x-data="{ contractType: '{{ old('contract_type', $contract->contract_type ?? 'sewa') }}' }"
    >
        @csrf
        @method('PUT')
        ...
        <div class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl p-6">
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
                ...
                <div class="sm:col-span-3" x-show="contractType === 'sewa'">
        ...
        <div x-show="contractType === 'ksu'" x-transition x-cloak class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl p-6">
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
                ...
                <div class="col-span-full">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" x-data="{ sharingType: '{{ old('sharing_type', $contract->sharing_type ?? 'revenue_sharing') }}' }">
                        <label class="relative flex cursor-pointer rounded-lg border p-4 transition-colors"
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2 relative rounded-md shadow-sm">
                        <input type="number" name="company_share_pct" id="company_share_pct" value="{{ old('company_share_pct', $contract->company_share_pct) }}" step="0.01" min="0" max="100" class="block w-full rounded-md border-0 py-1.5 pr-10 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-purple-600 sm:text-sm sm:leading-6" placeholder="e.g. 70">
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2 relative rounded-md shadow-sm">
                        <input type="number" name="tenant_share_pct" id="tenant_share_pct" value="{{ old('tenant_share_pct', $contract->tenant_share_pct) }}" step="0.01" min="0" max="100" class="block w-full rounded-md border-0 py-1.5 pr-10 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-purple-600 sm:text-sm sm:leading-6" placeholder="e.g. 30">
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                            <span class="text-gray-500 sm:text-sm">%</span>
                        </div>
                    </div>
                ...
                <div class="col-span-full">
                    <div class="rounded-md bg-purple-50 p-3 border border-purple-200">
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <p class="text-xs text-purple-700">Untuk KSU, pencatatan cash basis dilakukan melalui pembuatan Invoice manual dari hasil rekonsiliasi.</p>
                            </div>
                        </div>
                    </div>
                </div>
