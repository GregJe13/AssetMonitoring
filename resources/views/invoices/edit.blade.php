...
<div class="max-w-3xl mx-auto">
    <div class="md:flex md:items-center md:justify-between mb-8">
        <div>
            <h2 class="text-2xl font-bold leading-7 text-gray-900">Edit Invoice</h2>
            <p class="mt-1 text-sm text-gray-500">{{ $invoice->invoice_number }}</p>
        </div>
        <a href="{{ route('invoices.show', $invoice) }}" class="text-sm text-indigo-600 hover:text-indigo-800">← Kembali</a>
    </div>
    ...
    <form action="{{ route('invoices.update', $invoice) }}" method="POST" enctype="multipart/form-data"
          ...
          x-data="{
              tenantType: '{{ $invoice->tenant_id ? 'registered' : 'manual' }}',
              tenantName: '{{ old('tenant_name_manual', $invoice->tenant_name_manual ?? '') }}'
          }">
        @csrf
        @method('PUT')
        ...
        <div class="px-4 py-6 sm:p-8 space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" required
                    class="mt-2 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm">{{ old('description', $invoice->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <div class="relative mt-2">
                    ...
                    <input type="number" name="amount" id="amount" value="{{ old('amount', $invoice->amount) }}" required min="0" step="0.01"
                        class="block w-full rounded-md border-0 py-1.5 pl-10 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm">
                </div>
            </div>
                ...
                <div x-show="tenantType === 'registered'">
                    <select name="tenant_id" id="tenant_id"
                        ...
                        @foreach($tenants as $tenant)
                            <option value="{{ $tenant->id }}" {{ old('tenant_id', $invoice->tenant_id) == $tenant->id ? 'selected' : '' }}>{{ $tenant->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto border rounded-md p-3">
                    @php $selectedAssets = old('asset_ids', $invoice->assets->pluck('id')->toArray()); @endphp
                    @foreach($assets as $asset)
                        <label class="flex items-center gap-2 text-sm py-1 px-2 rounded hover:bg-gray-50 cursor-pointer">
                            <input type="checkbox" name="asset_ids[]" value="{{ $asset->id }}"
                                {{ in_array($asset->id, $selectedAssets) ? 'checked' : '' }}
            ...
            <div>
                ...
                @if($invoice->file_path)
                    <div class="mt-2 mb-2 flex items-center gap-2 text-sm text-gray-600">
                        <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        <a href="{{ asset('storage/' . $invoice->file_path) }}" target="_blank" class="text-indigo-600 hover:text-indigo-800">{{ $invoice->file_original_name }}</a>
                        <span class="text-gray-400">(upload baru akan mengganti file ini)</span>
                    </div>
                @endif
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="2"
                    class="mt-2 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm">{{ old('notes', $invoice->notes) }}</textarea>
            </div>
        </div>

        {{-- Submit --}}
        <div class="flex items-center justify-end gap-x-4 border-t border-gray-900/10 px-4 py-4 sm:px-8">
            <a href="{{ route('invoices.show', $invoice) }}" class="text-sm font-semibold text-gray-900">Batal</a>
